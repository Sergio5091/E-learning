    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="flex justify-between items-center mb-8">
        <button @click="previousLesson" :disabled="currentLessonIndex === 0"
          class="px-4 py-2 text-sm font-medium text-gray-700 bg-white rounded-md border border-gray-300 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">Précédent</button>
        <span class="text-sm font-medium text-gray-600">Leçon {{ currentLessonIndex + 1 }} sur {{ course.lessons.length
          }}</span>
        <button @click="nextLesson" :disabled="currentLessonIndex === course.lessons.length - 1"
          class="px-4 py-2 text-sm font-medium text-gray-700 bg-white rounded-md border border-gray-300 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">Suivant
        </button>
      </div>
      ...
      <div class="bg-white shadow-md rounded-lg">
        ...
        <ul>
          <li v-for="(lesson, index) in course.lessons" :key="lesson.id"
            class="border-b hover:bg-red-50 transition-colors duration-500 cursor-pointer border-gray-200 last:border-b-0 last:pb-4 ">
            <div @click="selectLesson(index)"
              class="flex justify-between items-center p-4 cursor-pointer hover:bg-gray-100"
              :class="{ 'bg-gray-100': currentLessonIndex === index }">
            ...
            <div v-if="openedLessonId === lesson.id" class="p-4 bg-gray-50 border-t">
              ...
              <div class="flex items-center">
                ...
                <label :for="'lesson-' + lesson.id" class="ml-2 block text-sm text-gray-900">Marquer comme
                  terminé</label>
              </div>
            </div>
          </li>
